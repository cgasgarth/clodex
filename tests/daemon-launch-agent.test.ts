...
describe('daemon launch agent', () => {
  ...
  it('uses a stable proxy port so sessions reconnect after daemon restarts', () => {
  ...
  it('uses a distinct stable endpoint port for OpenAI-only Claude sessions', () => {
    expect(resolveDaemonEndpointPort({})).toBe(DEFAULT_DAEMON_ENDPOINT_PORT);
    expect(resolveDaemonEndpointPort({ CLODEX_DAEMON_ENDPOINT_PORT: '27778' })).toBe(27778);
    expect(() => resolveDaemonEndpointPort({ CLODEX_DAEMON_ENDPOINT_PORT: '0' }))
      .toThrow('between 1 and 65535');
  });
});
