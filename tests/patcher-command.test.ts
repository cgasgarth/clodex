...
describe('runPatchCommand version resolution', () => {
  it('patches the resolved install and never downgrades it to a PATH shim\'s version', async () => {
    ...
    // the shim's version restored 2.1.215's bytes over the 2.1.233 install.
    const real = installClaude('2.1.233');
    ...
    // The install is still 2.1.233 — not overwritten with 2.1.215's bytes.
    expect(versionOf(real)).toBe('2.1.233');
    ...
    expect(manifest?.claudeVersion).toBe('2.1.233');
    expect(manifest?.binaryPath).toBe(real);

    // The pristine snapshot is the 2.1.233 binary, stored under its content address.
    const backup = manifest!.backupPath;
    expect(backup).toMatch(/claude-2\.1\.233-[0-9a-f]{16}\.orig$/);
  ...
  it('takes the version from TWEAKCC_CC_INSTALLATION_PATH\'s binary, not from PATH', async () => {
    const target = join(home, 'opt', 'claude-2.1.233');
    writeFakeClaude(target, '2.1.233');
    ...
    expect(await runPatchCommand({})).toBe(0);
    expect(readPatchManifest()?.claudeVersion).toBe('2.1.233');
    expect(backupFiles()).toContainEqual(expect.stringMatching(/^claude-2\.1\.233-[0-9a-f]{16}\.orig$/));
  });
  ...
  it('refuses an unsupported Claude Code version before reading or modifying its bundle', async () => {
    ...
    expect(logs.join('\n')).toContain('Clodex supports patching Claude Code 2.1.233; found 2.1.225');
...
describe('runPatchCommand pristine backup safety', () => {
  ...
  it('never stores an already-patched binary as the pristine backup', async () => {
    ...
    const real = installClaude('2.1.233', patchedBundle);
    const before = readFileSync(real);
  ...
  it('refuses a legacy backup whose bytes belong to another version', async () => {
    ...
    const real = installClaude('2.1.233', `${PRISTINE_BUNDLE}\n/*ccpatch:ctx*/var _ccw=({})[""];`);
    const before = readFileSync(real);
    mkdirSync(tweakccDir, { recursive: true });
    writeFakeClaude(join(tweakccDir, 'claude-2.1.233.orig'), '2.1.215');
...
describe('runPatchCommand legacy backup compatibility', () => {
  it('adopts an existing claude-<ver>.orig backup instead of orphaning it', async () => {
    ...
    const pristineBytes = readFileSync(real);
    mkdirSync(tweakccDir, { recursive: true });
    const legacy = join(tweakccDir, 'claude-2.1.233.orig');
    ...
    const manifest = readPatchManifest()!;
    expect(manifest.backupPath).toMatch(/claude-2\.1\.233-[0-9a-f]{16}\.orig$/);
  ...
  it('re-seeds from a content-addressed backup when the live binary turns out to be patched', async () => {
    ...
    const pristineBytes = (() => {
      ...
      writeFakeClaude(staging, '2.1.233');
      return readFileSync(staging);
    })();
    const real = installClaude('2.1.233', `${PRISTINE_BUNDLE}\n/*ccpatch:ctx*/var _ccw=({})[""];`);
    mkdirSync(tweakccDir, { recursive: true });
    const backup = join(tweakccDir, `claude-2.1.233-${sha256OfBuffer(pristineBytes).slice(0, 16)}.orig`);
  ...
  it('restores a patched binary from a legacy backup that proves its version', async () => {
    ...
    const real = installClaude('2.1.233', `${PRISTINE_BUNDLE}\n/*ccpatch:ctx*/var _ccw=({})[""];`);
    mkdirSync(tweakccDir, { recursive: true });
    writeFileSync(join(tweakccDir, 'claude-2.1.233.orig'), legacyBytes, { mode: 0o755 });
...
describe('runPatchCommand --restore', () => {
  ...
  it('still restores when the binary is too broken to report its version', async () => {
    ...
    expect(versionOf(real)).toBe('2.1.233');
    expect(readPatchManifest()).toBeNull();
    expect(logs.join('\n')).toMatch(/using claude 2\.1\.233 from the patch manifest/);
  });
...
describe('runPatchCommand poisoned backup safety', () => {
  ...
  function poisonedBackupBytes(): Buffer {
    ...
    writeFakeClaude(staging, '2.1.233', `${PRISTINE_BUNDLE}\n/*ccpatch:effort*/var _ccc={};`);
    return readFileSync(staging);
  }
  ...
  it('refuses to patch from a backup whose bytes are already patched', async () => {
    ...
    mkdirSync(tweakccDir, { recursive: true });
    const backup = join(tweakccDir, `claude-2.1.233-${sha256OfBuffer(poisoned).slice(0, 16)}.orig`);
  ...
  it('does not adopt a poisoned legacy backup into a content-addressed name', async () => {
    ...
    const poisoned = poisonedBackupBytes();
    mkdirSync(tweakccDir, { recursive: true });
    writeFileSync(join(tweakccDir, 'claude-2.1.233.orig'), poisoned, { mode: 0o755 });

    expect(await runPatchCommand({})).toBe(1);
    expect(logs.join('\n')).toMatch(/already carry a clodex patch/);
    expect(sha256Of(real)).toBe(before);
    expect(backupFiles()).toEqual(['claude-2.1.233.orig']);
  });
});
...
describe('runPatchCommand backup directory integrity', () => {
  it('replaces a corrupt file squatting the content address instead of adopting it', async () => {
    ...
    const pristineBytes = readFileSync(real);
    mkdirSync(tweakccDir, { recursive: true });
    writeFileSync(join(tweakccDir, 'claude-2.1.233.orig'), pristineBytes, { mode: 0o755 });
    const canonical = join(tweakccDir, `claude-2.1.233-${sha256OfBuffer(pristineBytes).slice(0, 16)}.orig`);
