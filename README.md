...
The patch map is built from your favorites and aliases; context windows come from provider metadata. This Clodex release supports Claude Code **2.1.237 only**. `clodex patch` rejects every other version before it reads, backs up, or changes the binary. `clodex patch --restore` remains available for recovery.
