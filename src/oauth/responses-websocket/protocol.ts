...
function diagnosticItemIdHash(value: JsonValue): string | undefined {
  return isString(value) && value.length > 0
    ? createHash('sha256').update(value).digest('hex').slice(0, 16)
    : undefined;
}

function reasoningPartIndex(value: JsonValue): number | undefined {
...
function emitProtocolAnomaly(
  ...
  const parts = isString(itemId) ? ctx.reasoningPartsByItemId.get(itemId) : undefined;
  emitContextDiagnostic(entry, ctx, {
    ...
    knownSummaryParts: parts
      ? [...parts.entries()].toSorted(([left], [right]) => left - right)
...
export function trackReasoningProtocol(
  ...
  if (!type || !isJsonObject(event)) return;
  ...
  if (type === 'response.output_item.added' || type === 'response.output_item.done') {
    ...
    if (!isString(itemId) || itemId.length === 0) return;
  ...
  if (!isString(itemId) || summaryIndex === undefined) return;
