import type { HttpResponseWriter } from '../transport/http-utils.js';
...
interface CreateAgentStreamTransactionOptions {
  enabled: boolean;
  response: Pick<HttpResponseWriter, 'headersSent' | 'writeHead' | 'write'>;
  onOutput: (chunk: string) => void;
  onBufferLimitExceeded: (bufferedBytes: number) => void;
}

interface CreatedAgentStreamTransaction {
  transaction: AgentStreamTransaction;
  ensureHeaders: () => void;
}

export function createAgentStreamTransaction(
  options: CreateAgentStreamTransactionOptions,
): CreatedAgentStreamTransaction {
  const ensureHeaders = () => {
