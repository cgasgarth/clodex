// src/patcher/transforms.ts — clodex patch transforms, applied in-process.
//
...
import { isReservedModelAlias } from '../models/aliases.js';
...
export function projectNativeEffort(
  ...
  if (!effort || !Array.isArray(effort.levels) || !isString(effort.defaultLevel)) return undefined;
...
function capabilityKeys(value: string): string[] {
  const normalized = value.trim().toLowerCase();
  const bare = normalized.replace(/\[1m\]$/i, '');
  return [...new Set([bare, `${bare}[1m]`])];
}

function reEsc(value: string): string {
...
export function applyClodexPatches(source: string, config: PatchScriptModelConfig): ApplyPatchesOutcome {
  ...
  // SAFETY: This null-prototype object stores only validated patch configuration entries.
  ...
  for (const [id, value] of Object.entries(MODEL_CONFIG)) {
    ...
    if (spec.alias !== undefined) {
      const rawAlias = spec.alias.trim();
      ...
      ALIAS_TO_ID[a] = id;
      IDENTITIES.push(a);
      if (spec.display) DISPLAY_BY_IDENTITY[a] = spec.display;
    } else {
      IDENTITIES.push(id);
      if (spec.display) DISPLAY_BY_IDENTITY[id] = spec.display;
    }
    if (spec.alias !== undefined) {
      registerCapabilityKeys(spec.alias);
    }
    registerCapabilityKeys(id);
    if (id.startsWith('clodex:openai-oauth:')) {
      const upstreamId = id.slice('clodex:openai-oauth:'.length);
      if (spec.alias !== undefined) {
        for (const key of capabilityKeys(spec.alias)) FAST_MODE_KEYS.add(key);
      }
      for (const key of capabilityKeys(id)) FAST_MODE_KEYS.add(key);
      for (const key of capabilityKeys(upstreamId)) FAST_MODE_KEYS.add(key);
    }

    if (spec.context !== undefined) {
      const n = spec.context;
      ...
      if (/\[1m\]/i.test(spec.alias ?? '') || /\[1m\]/i.test(id)) {
        fail(
          'clodex patch: "' + id + '" sets context but keeps the [1m] suffix — drop the suffix from both the id and the alias'
        );
      }
      if (spec.alias !== undefined) CONTEXT_BY_KEY[spec.alias.trim().toLowerCase()] = n;
      CONTEXT_BY_KEY[id.trim().toLowerCase()] = n;
    }
    ...
    if (spec.effort) {
      ...
      if (spec.alias !== undefined) {
        for (const key of capabilityKeys(spec.alias)) {
          EFFORT_BY_KEY[key] = effort;
        }
      }
      for (const key of capabilityKeys(id)) {
        EFFORT_BY_KEY[key] = effort;
      }
    }
  }
  ...
  // The picker is assembled through one choke-point function. Claude 2.1.245
  // starts it from the built-in rows, then reads ANTHROPIC_CUSTOM_MODEL_OPTION;
  // append our custom
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    const entries = missing
      ...
      .join(',');
    const inject = (options: string): string => missing.length
      ? '[' + entries + '].forEach(function(_o){if(!' + options
        + '.some(function(_i){return _i.value===_o.value}))' + options + '.push(_o)});'
      : '';
    if (ALIASES.length === 0) {
      ...
      applyOnce(
        ...
        /(function [\w$]+\([\w$]+,[\w$]+\)\{let ([\w$]+)=[\w$]+\([\w$]+\),[\w$]+=[\w$]+\.ANTHROPIC_CUSTOM_MODEL_OPTION;)/,
        (head, options) => head + '/*ccpatch:model-picker*/' + inject(options),
        { marker: '/*ccpatch:model-picker*/', required: false, noopIsSkip: true }
      );
    }
  }
  ...
  // Append the available model names (with their real labels) so the model knows
  // which extra names it may request and what they are. Best-effort (cosmetic).
  // ---------------------------------------------------------------------------
  {
    ...
    applyOnce(
      ...
      /(describe\()("Model override for this agent\. Use `inherit` to match the spawning conversation\.")(\))/,
      (_m, open, body, close) =>
        open + '/*ccpatch:agent-model-description*/'
        + q(JSON.parse(body) + ' Additional custom models: ' + listing + '.') + close,
      { marker: '/*ccpatch:agent-model-description*/', required: false, noopIsSkip: true }
    );
  }
  ...
  // PATCH X2 — prevent Claude's local context guard from blocking a Clodex child.
  {
    ...
    applyOnce(
      ...
      /(function [\w$]+\([\w$]+,[\w$]+,[\w$]+,[\w$]+=[\w$]+,[\w$]+\)\{)(let [\w$]+=[\w$]+\?\?[\w$]+\([\w$]+,[\w$]+\),[\w$]+=[\w$]+\.enabled\?[\w$]+:[\w$]+,[\w$]+=[\w$]+-20000,[\w$]+=[\w$]+\.testBlockingOverride,)/,
      (_m, head, body) =>
        head + MARKER
  ...
  // PATCH X3 — suppress Claude's background precomputed auto-compaction arm.
  {
    ...
    applyOnce(
      ...
      /(function [\w$]+\([\w$]+,[\w$]+,[\w$]+,[\w$]+\)\{)(let [\w$]+=[\w$]+\([\w$]+,[\w$]+,[\w$]+\),[\w$]+=[\w$]+\.enabled\?[\w$]+:void 0,[\w$]+=[\w$]+\([\w$]+,[\w$]+\);if\(![\w$]+\([\w$]+,[\w$]+\)\)return [\w$]+>=[\w$]+\([\w$]+,[\w$]+\);)/,
      (_m, head, body) =>
        head + MARKER
        + 'if(process.env.CLODEX_NATIVE_CONTEXT_OWNER==="1")return!1;'
        + body,
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    applyOnce(
      ...
      /(function [\w$]+\([\w$]+,[\w$]+,[\w$]+=[\w$]+\(\)\)\{let [\w$]+=[\w$]+\([\w$]+\),([\w$]+)=[\w$]+\([\w$]+,[\w$]+\);)(if\(process\.env\.CLAUDE_CODE_AUTO_COMPACT_WINDOW\))/,
      (_m, head, modelCap, envBranch) =>
        head + MARKER
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    applyOnce(
      ...
      (_m, predicate, providerKind, configVar) =>
        'function ' + predicate + '(){/*ccpatch:fast-provider*/if(' + envGate + ')return!'
        + configVar + '.CLAUDE_CODE_DISABLE_FAST_MODE;if(' + providerKind
        + '()!=="firstParty")return!1;return!' + configVar + '.CLAUDE_CODE_DISABLE_FAST_MODE}',
    ...
    applyOnce(
      ...
      /(function [\w$]+\([\w$]+,[\w$]+,([\w$]+)=!0,[\w$]+\)\{)(if\([\w$]+\(\),!\2\))/,
      (_m, head, persist, body) =>
        head + '/*ccpatch:fast-session-write*/if(' + envGate + ')' + persist + '=!1;' + body,
      { marker: '/*ccpatch:fast-session-write*/', required: true },
    );

    applyOnce(
      'PATCH X9e: Clodex Fast session-only status',
      /(async function [\w$]+\([\w$]+,[\w$]+,[\w$]+,[\w$]+,([\w$]+)=!0,[\w$]+\)\{)(let [\w$]+=[\w$]+\(\);if\([\w$]+\)return`Fast mode unavailable: \$\{[\w$]+\}`;)/,
      (_m, head, persist, body) =>
        head + '/*ccpatch:fast-session-status*/if(' + envGate + ')' + persist + '=!1;' + body,
