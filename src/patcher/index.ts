// src/patcher/index.ts — clodex patch: first-class Claude Code binary patcher.
//
...
import { findClaudeBinary, getClaudeVersionForBinary } from '../runtime/launch.js';
import {
  backupDir,
...
import { stripOneMContextSuffix } from '../models/context-model-id.js';
...
const SUPPORTED_CLAUDE_CODE_VERSION = '2.1.234';
...
export function readPatchManifest(path = getPatchManifestPath()): PatchManifest | null {
  try {
    // SAFETY: The required manifest identity fields are checked before this value is returned.
    const parsed = JSON.parse(readFileSync(path, 'utf8')) as PatchManifest;
    if (isString(parsed.binaryPath) && isString(parsed.configHash)) {
      return parsed;
    }
  } catch {
...
export function buildPatchModelConfig(
  ...
  aliases: StoredModelAlias[] | undefined,
...
function pidIsAlive(pid: number): boolean {
  try {
    ...
    // SAFETY: Node system errors expose `code`; absent codes produce the normal false result.
...
export function tryAcquirePatchLock(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      let stale = false;
      try {
        // SAFETY: The lock is private Clodex state; its numeric fields are checked by use below.
        const existing = JSON.parse(readFileSync(lockPath, 'utf8')) as PatchLockContent;
        stale = !existing.pid
          || !isAlive(existing.pid)
          || (isNumber(existing.startedAt) && now - existing.startedAt > PATCH_LOCK_STALE_MS);
      } catch {
