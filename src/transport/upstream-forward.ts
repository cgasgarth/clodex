import type { HttpResponseWriter } from './http-utils.js';
...
export interface AnthropicUpstreamHeaders {
  [name: string]: string;
  'Content-Type': string;
...
export function anthropicUpstreamHeaders(
  ...
  if (resolvedAuthType !== 'none') {
    ...
    if (!isOAuth) headers['x-api-key'] = key;
  }
  if (isOAuth) {
    headers['User-Agent'] = CLAUDE_CODE_USER_AGENT;
    headers['x-app'] = 'cli';
  }
  if (isOAuth && claudeCodeSessionId) {
