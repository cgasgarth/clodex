// src/cli/providers-command.ts — clodex providers command
...
type ProvidersSubcommand = 'hub' | 'add' | 'list' | 'remove' | 'refresh-models' | 'auth' | 'help';
...
function resolveRegistryProviderKey(
  provider: import('../registry/types.js').RegistryProvider,
): Promise<string | null> {
...
async function runProvidersRefreshModels(providerId?: string): Promise<number> {
  if (providerId) {
    ...
    const key = await resolveRefreshCredential(provider, resolveRegistryProviderKey);
  ...
  const { refreshed } = await refreshAllProviderModels(resolveRegistryProviderKey);
  spinner.stop('');
...
async function runProvidersHub(): Promise<number> {
  while (true) {
    ...
    if (isString(choice) && choice.startsWith('provider:')) {
