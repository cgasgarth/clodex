...
import { fmtModel, fmtProviderBracket, formatModelLabel } from '../ui/prompts.js';
import { scoreModelSearch } from './search.js';
...
export async function pickGlobalFavoriteModel(
  ...
  while (true) {
    ...
    const matched = filterGlobalFavoriteIndex(index, searchInput, { freeOnly });
    if (matched.length === 0) {
