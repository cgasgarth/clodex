...
import type { ProxyModelAlias, ProxyRoute } from '../proxy/index.js';
import type {
  FavoriteModel,
  LocalProvider,
  LocalProviderModel,
  ...
  return [
    ...normalized.accepted.map(({ alias, source, sources }) => {
      ...
      if (sourceNames.length !== 1 || sourceNames[0] !== alias.name) {
        entry.sourceNames = sourceNames;
      }
      if (collidesWithCatalog) {
        entry.unavailableReason = 'conflicts with a catalog model id';
      } else if (!route) {
        entry.unavailableReason = 'target unavailable';
      }
      return entry;
    }),
 ...
 */
export interface CatalogRouteBuild {
  routes: ProxyRoute[];
