...
// Derived from Claude Code v2.1.240's built-in model resolver and sentinels.
...
function isStoredAliasCarrier(
  value: JsonValue | ModelAlias | StoredModelAlias,
): value is StoredModelAlias {
...
export function modelAliasMatchesName(value: JsonValue | StoredModelAlias | ModelAlias, name: string): boolean {
  if (!value || !isObject(value) || Array.isArray(value) || !('name' in value)) return false;
  const candidate = value.name;
  return isString(candidate)
    && canonicalModelAliasName(candidate) === canonicalModelAliasName(name);
}

export function modelAliasMatchesStoredName(value: JsonValue | StoredModelAlias | ModelAlias, name: string): boolean {
  if (!value || !isObject(value) || Array.isArray(value) || !('name' in value)) return false;
  const candidate = value.name;
...
export function normalizeModelAliases(
  ...
  for (const [index, item] of value.entries()) {
    ...
    if (
      !isString(source.providerId)
      || !isString(source.modelId)
    ) {
