...
interface CredentialHelperTimeoutState {
...
export function configuredCredentialHelperPath(): string | null {
  ...
  try {
    ...
    throw new Error(`${CREDENTIAL_HELPER_ENV} is not an executable file`, { cause: err });
  }
  return value;
}
...
async function runCredentialHelper(
  ...
  if (!stdin || !(stdin instanceof Object) || !stdout || !(stdout instanceof Object)) {
    child.kill(9);
    throw new Error(`credential helper ${operation} could not start`);
  }
  ...
  const timeoutState: CredentialHelperTimeoutState = {};
  ...
  try {
    const [code, helperStdout] = await Promise.race([completion, timeout]);
    return { code, stdout: helperStdout };
  } finally {
